		...
		switch ($_GET["op"]) {
			...
			case 'descarga':
				...
				while ($reg = $rspta->fetch_object()) {
					...
					'ASIG_FAMILIAR'=>$reg->pasig_familiar,
					'CARGO'=>$reg->cargo,
					'SUB_AREA'=>$reg->subarea,
					'AREA'=>$reg->area,
					'CENTRO_COSTO'=>$reg->pcentro_costo,
					'FECHA_INGRESO'=>$reg->fecha_ingreso,
					'FECHA_CESE'=>$reg->fecha_cese,
					// 'MOTIVO_CESE'=>$reg->,
					'SEXO'=>$reg->psexo,
					'NACIONALIDAD'=>$reg->pnacionalidad,
					'FECHA_NACIMIENTO'=>$reg->FECHA_NACIMIENTO,
					'ESTADO_CIVIL'=>$reg->pestado_civil,
					'CELULAR'=>$reg->cell,
					...
					'SCTR_SALUD'=>$reg->psctr_salud,
					'SCTR_PENSION'=>$reg->psctr_pension,
					'PLANILLA'=>$reg->pplanilla,
					'EPS_PLAN'=>$reg->EPS_PLAN,
					'EPS'=>$reg->peps,
					'JEFE_CARGO'=>$reg->nombre_jefe
					];
					$cont++;
				}
