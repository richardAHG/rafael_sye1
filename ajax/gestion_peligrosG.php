...
if (!isset($_SESSION["nombre"])) {
	...
	if ($_SESSION['Administrar'] == 1) {
		...
		$gestionPeligros = new GestionPeligrosG();
		$TIPO_GENERAL = 1;
		...
		switch ($_GET["op"]) {
			case 'guardaryeditar':
				if (empty($id)) {
					$rspta = $gestionPeligros->insertar($tipo_ats_id, $peligroG_id, $riesgoG_id, $medidaG_id, $respuesta);
					...
					$rspta = $gestionPeligros->editar($id, $tipo_ats_id, $peligroG_id, $riesgoG_id, $medidaG_id, $respuesta);
			...
			case "selecPG":
				...
				$obj = new Peligro();
				$rspta = $obj->selectP($TIPO_GENERAL);
				$data = [];
				while ($reg = $rspta->fetch_object()) {
					$data[] = [
						'id' => $reg->id,
			...
			case "selecRG":
				...
				$obj = new Riesgo();
				$rspta = $obj->selectR($TIPO_GENERAL);
				$data = [];
				while ($reg = $rspta->fetch_object()) {
					$data[] = [
						'id' => $reg->id,
			...
			case "selecMG":
				...
				$rspta = $obj->selectM($TIPO_GENERAL);
				$data = [];
				while ($reg = $rspta->fetch_object()) {
					$data[] = [
						'id' => $reg->id,
