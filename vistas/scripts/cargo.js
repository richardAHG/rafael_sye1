...
//Función Listar
function listar() {
  let url = "../ajax/cargo.php?op=listar";
...
function mostrar(id) {
  $.post("../ajax/cargo.php?op=mostrar", { id: id }, function (data, status) {
...
function desactivar(id) {
  bootbox.confirm("¿Está Seguro de desactivar el Cargo?", function (result) {
    if (result) {
      $.post("../ajax/cargo.php?op=desactivar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable('refresh');
      });
...
function activar(id) {
  bootbox.confirm("¿Está Seguro de activar el cargo?", function (result) {
    if (result) {
      $.post("../ajax/cargo.php?op=activar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable('refresh');
      });
