...
function init() {
  ...
  $.post("../ajax/peligro.php?op=selectTipo", function (r) {
    data = JSON.parse(r);
    options = data.data;
...
//Función Listar
function listar() {
  let url = "../ajax/peligro.php?op=listar";
...
function guardaryeditar(e) {
  ...
  $.ajax({
    url: "../ajax/peligro.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function mostrar(id) {
  $.post("../ajax/peligro.php?op=mostrar", { id: id }, function (data, status) {
...
function desactivar(id) {
  bootbox.confirm("¿Está Seguro de desactivar el Peligro?", function (result) {
    if (result) {
      $.post("../ajax/peligro.php?op=desactivar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable('refresh');
      });
...
function activar(id) {
  bootbox.confirm("¿Está Seguro de activar el Peligro?", function (result) {
    if (result) {
      $.post("../ajax/peligro.php?op=activar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable('refresh');
      });
