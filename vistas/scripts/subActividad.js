...
//Función Listar
function listar() {
  let url = "../ajax/subActividad.php?op=listar";
...
function guardaryeditar(e) {
  ...
  $.ajax({
    url: "../ajax/subActividad.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function mostrar(id) {
  $.post(
    "../ajax/subActividad.php?op=mostrar",
    { id: id },
...
function desactivar(id) {
  bootbox.confirm(
    "¿Está Seguro de desactivar el Sub Actividad?",
    function (result) {
      if (result) {
        $.post(
          "../ajax/subActividad.php?op=desactivar",
          { id: id },
          function (e) {
            data = JSON.parse(e);
            bootbox.alert(data.mensaje);
...
function activar(id) {
  bootbox.confirm(
    "¿Está Seguro de activar el Sub Actividad?",
    function (result) {
      if (result) {
        $.post("../ajax/subActividad.php?op=activar", { id: id }, function (e) {
          data = JSON.parse(e);
          bootbox.alert(data.mensaje);
          $("#tbllistado").bootstrapTable("refresh");
        });
