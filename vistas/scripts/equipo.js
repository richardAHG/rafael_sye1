...
function init() {
  ...
  $.post("../ajax/equipo.php?op=selectTipo", function (r) {
    data = JSON.parse(r);
    options = data.data;
...
//Función Listar
function listar() {
  let url = "../ajax/equipo.php?op=listar";
...
function mostrar(id) {
  $.post("../ajax/equipo.php?op=mostrar", { id: id }, function (data, status) {
...
function desactivar(id) {
  bootbox.confirm("¿Está Seguro de desactivar el Equipo?", function (result) {
    if (result) {
      $.post("../ajax/equipo.php?op=desactivar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable("refresh");
      });
...
function activar(id) {
  bootbox.confirm("¿Está Seguro de activar el Equipo?", function (result) {
    if (result) {
      $.post("../ajax/equipo.php?op=activar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable("refresh");
      });
