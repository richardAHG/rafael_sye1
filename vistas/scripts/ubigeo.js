...
//Función Listar
function listar() {
    let url = "../ajax/ubigeo.php?op=listar";
...
function guardaryeditar(e) {
    ...
    $.ajax({
        url: "../ajax/ubigeo.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id) {
    ...
    $.post("../ajax/ubigeo.php?op=mostrar", { id: id }, function(data, status) {
...
function desactivar(id) {
    bootbox.confirm("¿Está Seguro de desactivar el ubigeo?", function(result) {
        if (result) {
            $.post("../ajax/ubigeo.php?op=desactivar", { id: id }, function(e) {
                data = JSON.parse(e);
                bootbox.alert(data.mensaje);
                // tabla.ajax.reload();
                $("#tbllistado").bootstrapTable('refresh');
...
function activar(id) {
    bootbox.confirm("¿Está Seguro de activar el Area?", function(result) {
        if (result) {
            $.post("../ajax/ubigeo.php?op=activar", { id: id }, function(e) {
                data = JSON.parse(e);
                bootbox.alert(data.mensaje);
                // tabla.ajax.reload();
                $("#tbllistado").bootstrapTable('refresh');
