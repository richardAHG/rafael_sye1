...
function init() {
  ...
  $.post("../ajax/riesgo.php?op=selectTipo", function (r) {
    data = JSON.parse(r);
    options = data.data;
...
//Función Listar
function listar() {
  let url = "../ajax/riesgo.php?op=listar";
...
function guardaryeditar(e) {
  ...
  $.ajax({
    url: "../ajax/riesgo.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function mostrar(id) {
  $.post("../ajax/riesgo.php?op=mostrar", { id: id }, function (data, status) {
...
function desactivar(id) {
  bootbox.confirm("¿Está Seguro de desactivar el Riesgo?", function (result) {
    if (result) {
      $.post("../ajax/riesgo.php?op=desactivar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable("refresh");
      });
...
function activar(id) {
  bootbox.confirm("¿Está Seguro de activar el Riesgo?", function (result) {
    if (result) {
      $.post("../ajax/riesgo.php?op=activar", { id: id }, function (e) {
        data = JSON.parse(e);
        bootbox.alert(data.mensaje);
        $("#tbllistado").bootstrapTable("refresh");
      });
